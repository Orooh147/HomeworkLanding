...
public class LandingPhysics : MonoBehaviour
{
    ...
    {
        float bonusHeightText = 0.1f;
        RaycastHit2D raycasthit = Physics2D.BoxCast(cd.bounds.center, cd.bounds.size, 0f, Vector2.down, bonusHeightText, LandingLayer);

        if (Physics2D.BoxCast(cd.bounds.center, cd.bounds.size, 0f, Vector2.down, bonusHeightText, NotInZone))
        {
            isInZone = false;
        }
        else
